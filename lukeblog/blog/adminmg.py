...
import mgadmin
from mgadmin.layout import Row, Fieldset, Container  # mgadmin的表单支持
from mgadmin.filters import manager  # 自定义过滤器
from mgadmin.filters import RelatedFieldListFilter  # 自定义过滤器基类（自定义过滤器继承于该类）

from lukeblog.base_mgadmin import BaseOwnerMgadmin  # 导入admin中共有的配置，本文件中的admin类应当继承于此类
...
@mgadmin.sites.register(Category)
class CategoryAdmin(BaseOwnerMgadmin):
...
# mgadmin.site.register(Category, CategoryAdmin)  # 注册传入Model, AdminView


@mgadmin.sites.register(Tag)
class TagAdmin(BaseOwnerMgadmin):
    list_display = ('name', 'status', 'created_time')
    fields = ('name', 'status')


# mgadmin.site.register(Tag, TagAdmin)  # 注册传入Model, AdminView
...
@mgadmin.sites.register(Post)
class PostAdmin(BaseOwnerMgadmin):
...
# mgadmin.site.register(Post, PostAdmin)  # 注册传入Model, AdminView
