...
from django.template.loader import render_to_string  # 传入(模板, 字典)渲染成HTML
...
class SideBar(models.Model):
    ...
    @property  # 将content_html作为SideBar的属性对外暴露
    def content_html(self):
        """ 直接渲染成HTML """
        # 注意：在Model中引入其他Model时，在方法内部引入，可避免循环引用
        from blog.models import Post
        from comment.models import Comment

        result = ''  # 用来返回的HTML结果
        if self.display_type == self.DISPLAY_HTML:
            result = self.content  # 自定义HTML
        elif self.display_type == self.DISPLAY_LATEST:
            context = {
                'posts': Post.latest_posts(with_related=False)
            }
            result = render_to_string('config/blocks/sidebar_posts.html', context)  # 传入(模板, 字典)来渲染HTML
